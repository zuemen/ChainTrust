...
import { ensureHolderKeys } from "../../wallet/src/keys.ts";
...
assert(health.ok, `伺服器連線（chainMode=${health.chainMode}）`);
assert(health.chtIssuerDid && health.chtIssuerDid !== health.issuerDid, "雙簽發者：銀行 A 與中華電信為不同 DID");
...
// 3) nonce → 本機簽 KB → 驗證（requireKeyBinding）
const { nonce, aud } = await post("/sdjwt/nonce", {});
const parsed = parseSdJwt(vc);
const presentation = buildPresentationWithKeyBinding(parsed, ["kycLevel"], { aud, nonce });
...
const replay = await post("/sdjwt/verify", {
  presentation, tx: {}, requireKeyBinding: true, expectedNonce: nonce,
});
...
assert(mobIssuer === health.chtIssuerDid, "門號實名憑證由第二簽發者（中華電信）簽發");
